...
export function rankVaults(
  ...
  const maxApy = Math.max(...vaults.map((v) => v.analytics.apy.total), 1);
  const maxTvl = Math.max(...vaults.map((v) => Number(v.analytics.tvl.usd)), 1);

  return [...vaults].sort((a, b) => {
    const scoreA =
      (a.analytics.apy.total / maxApy) * w.apy + (Number(a.analytics.tvl.usd) / maxTvl) * w.tvl;
    const scoreB =
      (b.analytics.apy.total / maxApy) * w.apy + (Number(b.analytics.tvl.usd) / maxTvl) * w.tvl;
